...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$form_err) {
        ...
        try {
            ...
            // Redirect to the summary page, passing the maintenance_id
            header("Location: task_completion_summary.php?maintenance_id=" . urlencode($selected_maintenance_id) . "&msg=" . urlencode($success_message));
            exit();
    ...
    <div class="container mt-5">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="task_details" name="task_details" rows="5" required <?php if ($form_err) echo "disabled"; ?>><?php echo htmlspecialchars($task_details); ?></textarea>
                <span class="error-message"><?php echo $task_details_err; ?></span>
            </div>

            <div class="form-group">
                <label for="recommendations">Recommendations (e.g., next service, parts to monitor):</label>
                <textarea class="form-control" id="recommendations" name="recommendations" rows="3" <?php if ($form_err) echo "disabled"; ?>><?php echo htmlspecialchars($recommendations); ?></textarea>
            </div>

            <div class="form-group">
                <label for="completion_date">Completion Date:</label>
                <input type="date" class="form-control" id="completion_date" name="completion_date" required value="<?php echo htmlspecialchars($completion_date); ?>" <?php if ($form_err) echo "disabled"; ?>>
                <span class="error-message"><?php echo $completion_date_err; ?></span>
            </div>

            <button type="submit" class="btn btn-success" <?php if ($form_err) echo "disabled"; ?>>
